...
static struct __stream __primo_in, __primo_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__primo_in, "./primo_in", O_WRONLY);
	openstream(&__primo_out, "./primo_out", O_RDONLY);
}
...
static void __message_loop_primo(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__primo_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x71651df7u: {
				...
				uint32_t __cookie;
				int primo_posicion;
				readfull(&__primo_out, &primo_posicion, sizeof(int), 1);
				readfull(&__primo_out, &__cookie, sizeof(__cookie), 1);

				int result =
				primo(primo_posicion);

				writefull(&__primo_in, &__msgid, sizeof(__msgid));
				writefull(&__primo_in, &result, sizeof(result));
				writefull(&__primo_in, &__cookie, sizeof(__cookie));
				writeflush(&__primo_in);
				break;
			}
...
int busca(int n) {
	...
	writefull(&__primo_in, &__msgid, sizeof(__msgid));
	writefull(&__primo_in, &n, sizeof(int));
	writefull(&__primo_in, &__cookie, sizeof(__cookie));
	writeflush(&__primo_in);

	__message_loop_primo(__msgid,  0 );
	int __ans = 0;
	readfull(&__primo_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__primo_out, &__cookie_result, sizeof(__cookie_result), 1);
