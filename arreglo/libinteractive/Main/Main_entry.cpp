...
static struct __stream __cave_in, __cave_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__cave_in, "./cave_in", O_WRONLY);
	openstream(&__cave_out, "./cave_out", O_RDONLY);
}
...
static void __message_loop_cave(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__cave_out, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x71651df7u: {
				...
				uint32_t __cookie;
				int pos_P;
				readfull(&__cave_out, &pos_P, sizeof(int), 1);
				readfull(&__cave_out, &__cookie, sizeof(__cookie), 1);

				int result =
				pos(pos_P);

				writefull(&__cave_in, &__msgid, sizeof(__msgid));
				writefull(&__cave_in, &result, sizeof(result));
				writefull(&__cave_in, &__cookie, sizeof(__cookie));
				writeflush(&__cave_in);
				break;
			}
			...
			case 0x685d93e7u: {
				...
				uint32_t __cookie;
				int answer_N;
				readfull(&__cave_out, &answer_N, sizeof(int), 1);
				readfull(&__cave_out, &__cookie, sizeof(__cookie), 1);
...
void cuentaUnos(int N) {
	...
	writefull(&__cave_in, &__msgid, sizeof(__msgid));
	writefull(&__cave_in, &N, sizeof(int));
	writefull(&__cave_in, &__cookie, sizeof(__cookie));
	writeflush(&__cave_in);

	__message_loop_cave(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__cave_out, &__cookie_result, sizeof(__cookie_result), 1);
