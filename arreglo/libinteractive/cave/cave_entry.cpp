...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> cave.cuentaUnos
				uint32_t __cookie;
				int cuentaUnos_N;
				readfull(&__in, &cuentaUnos_N, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				cuentaUnos(cuentaUnos_N);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./cave_in", O_RDONLY);
	openstream(&__out, "./cave_out", O_WRONLY);
